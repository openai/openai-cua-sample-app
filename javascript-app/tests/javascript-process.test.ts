...
import { launchJavaScriptSession, type JavaScriptSession } from "../src/browser/javascript-process.js";

const workerPath = fileURLToPath(new URL("../src/javascript-worker.ts", import.meta.url));
...
async function start(executionTimeoutMs = 2_000, customWorker?: string, startPage = "index.html") {
  ...
  const session = await launchJavaScriptSession({
    ...
    url: pathToFileURL(join(directory, startPage)).href,
    targetLabel: startPage,
    executionTimeoutMs,
...
describe("persistent JavaScript worker", () => {
  ...
  it("rejects unknown operations after initialization", async () => {
    const directory = await mkdtemp(join(tmpdir(), "javascript-unknown-operation-"));
    directories.push(directory);
    const fixture = join(directory, "unknown-operation-worker.ts");
    ...
    `);
    const session = await start(2_000, fixture);
    await rememberWorker(session);
    await expect(session.readState()).rejects.toMatchObject({
      code: "javascript_worker_error",
      message: "Unknown JavaScript worker operation.",
    });
    for (const pid of pids) expect(isAlive(pid)).toBe(false);
  }, 15_000);

  it.each([
    { result: [{ type: "input_text", text: 42 }] },
    { result: [{ type: "input_image", image_url: "file:///tmp/image.png" }] },
  ])("rejects malformed execution IPC results and cleans up: %j", async ({ result }) => {
    const directory = await mkdtemp(join(tmpdir(), "javascript-output-protocol-"));
    directories.push(directory);
    const fixture = join(directory, "invalid-output-worker.mjs");
    await writeFile(fixture, `process.on("message", message => process.send({
      id: message.id,
      result: message.operation === "initialize" ? { currentUrl: "about:blank" } : ${JSON.stringify(result)},
    }));`);
    const session = await start(2_000, fixture);
    await expect(session.execute("console.log(1)")).rejects.toMatchObject({
      code: "javascript_worker_protocol_error",
      message: "JavaScript returned an unsupported output item.",
    });
    for (const pid of pids) expect(isAlive(pid)).toBe(false);
  }, 15_000);
