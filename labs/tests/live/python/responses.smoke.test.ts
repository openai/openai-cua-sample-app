...
import { RunnerManager } from "../../../../python-app/packages/runner-core/src/index.js";
...
beforeAll(() => {
  ...
  if (!process.env.OPENAI_API_KEY) {
    throw new Error(
      "OPENAI_API_KEY must be set to run labs/tests/live/python/responses.smoke.test.ts",
    );
  }
});
...
async function createLiveManager(stepDelayMs = 10) {
  ...
  const manager = new RunnerManager({ dataRoot, stepDelayMs });
...
describe("live Responses smoke", () => {
  it(
    ...
    async () => {
      ...
      expect(completed.run.status, completed.run.summary?.notes.join("\n")).toBe("completed");
  ...
  it(
    ...
    async () => {
      ...
      const detail = await manager.startRun({
        browserMode: "headful",
        prompt: "Draw a yellow smiley face with black eyes and a curved smile, then save the draft.",
