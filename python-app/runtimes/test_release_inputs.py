...
spec = importlib.util.spec_from_file_location("release", Path(__file__).with_name("release-inputs.py"))
...
class ReleaseTests(unittest.TestCase):
    def macos_backend(self, events, button_state=None):
        return SimpleNamespace(
            kCGHIDEventTap=0, kCGEventLeftMouseUp=1, kCGEventOtherMouseUp=2, kCGEventRightMouseUp=3,
            kCGMouseButtonLeft=0, kCGMouseButtonCenter=2, kCGMouseButtonRight=1,
            kCGEventSourceStateCombinedSessionState=0,
            CGEventCreate=lambda _: {}, CGEventGetLocation=lambda _: (0, 0),
            CGEventCreateKeyboardEvent=lambda _, key, down: {"key": key, "down": down},
            CGEventCreateMouseEvent=lambda _, kind, position, button: {
                "mouse": kind, "position": position, "flags": 123,
            },
    ...
    def test_macos_releases_at_a_corner_and_waits_between_mouse_events(self):
        ...
        with patch.dict(sys.modules, Quartz=quartz), patch.object(ctypes, "CDLL", return_value=services), \
    ...
    def test_macos_reports_when_a_mouse_release_was_not_observed(self):
        ...
        with patch.dict(sys.modules, Quartz=quartz), patch.object(ctypes, "CDLL", return_value=services), \
            ...
            with self.assertRaisesRegex(RuntimeError, "Mouse buttons remain held"):
                release.release_macos()
    ...
    def test_x11_flushes_releases_and_closes_the_display(self):
        ...
        xlib = SimpleNamespace(X=SimpleNamespace(KeyRelease=3, ButtonRelease=5),
                               display=SimpleNamespace(Display=lambda: connection))
    ...
    def test_releases_keys_and_all_buttons_without_other_operations(self):
        events = []
        release.release_inputs(["shift", "a", "shift"],
                               lambda key: events.append(("key_up", key)),
                               lambda button: events.append(("mouse_up", button)))
        self.assertEqual(events, [("key_up", "shift"), ("key_up", "a"),
                                  ("mouse_up", 1), ("mouse_up", 2), ("mouse_up", 3)])
    ...
    def test_windows_uses_only_native_up_events_including_modifiers(self):
        ...
        self.assertEqual([event for event in events if event[0] == "mouse"],
                         [("mouse", 4), ("mouse", 64), ("mouse", 16)])
