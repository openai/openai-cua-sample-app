...
spec = importlib.util.spec_from_file_location('worker', Path(__file__).with_name('python-worker.py'))
...
class WorkerTests(unittest.TestCase):
    ...
    def test_execute_protocol_preserves_globals_images_and_recovery(self):
        requests = [
            ...
            {"operation": "close"},
            {"id": 3, "operation": "execute", "code": 'counter = 99'},
        ]
    ...
    def test_removed_native_operations_do_not_execute_code(self):
        for operation in ("capture", "computer_actions", "cancel"):
            with self.subTest(operation=operation), patch.object(worker, "emit") as emit:
                namespace = {"counter": 0}
                request = {"id": 1, "operation": operation, "code": "counter = 99"}
    ...
    def test_import_from_an_unrelated_directory_with_safe_path_enabled(self):
        worker_path = str(Path(__file__).resolve().with_name("python-worker.py"))
        ...
        with tempfile.TemporaryDirectory() as directory:
            result = subprocess.run(
                [sys.executable, "-c", program, worker_path], cwd=directory,
                env=env, capture_output=True, text=True, timeout=10,
            )
        self.assertEqual(result.returncode, 0, result.stderr)

    def test_persistent_globals_and_error_recovery(self):
        namespace = {}
        first = worker.execute('counter = 41\nprint("first")\nraise ValueError("retry")', namespace)
        self.assertEqual(first['output'][0]['text'], 'first')
        self.assertIn('ValueError: retry', first['output'][-1]['text'])
        self.assertEqual(worker.execute('log(counter + 1)', namespace)['output'][0]['text'], '42')
    ...
    def test_image_output_is_png_with_original_detail(self):
        output = worker.execute('display(image)', {'image': Image.new('RGB', (13, 7))})['output'][0]
        self.assertEqual(output['detail'], 'original')
        self.assertTrue(base64.b64decode(output['image_url'].split(',')[1]).startswith(b'\x89PNG'))
