...
import { RunnerCoreError, RunnerManager } from "@cua-sample/runner-core";
...
describe("runner server", () => {
  ...
  it("starts, retrieves, stops, and resets scenario workspaces", async () => {
    ...
    const app = createServer({
      ...
      manager: new RunnerManager({
        dataRoot,
        executorFactory: () => ({
          execute: async ({ signal }) => new Promise<void>((resolve) => {
    ...
    try {
      ...
      expect(started.detail?.run.status).toBe("running");
      expect(startResponse.json().detail.run).not.toHaveProperty("mode");
      expect(startResponse.json().detail.scenario).not.toHaveProperty("defaultMode");
  ...
  it("rejects Headless runs before allocating a workspace", async () => {
    const app = createServer();
    try {
      const response = await app.inject({ method: "POST", url: "/api/runs", payload: {
        scenarioId: "fixture-scenario", browserMode: "headless", prompt: "Complete fixture.",
      } });
      expect(response.statusCode).toBe(400);
      expect(response.json().code).toBe("visible_browser_required");
...
describe("runner request boundaries", () => {
  it.each(["code", "native"])("rejects obsolete mode %s before calling the runner", async (mode) => {
    const startRun = vi.fn();
    const app = createServer({ manager: { startRun, shutdown: async () => undefined } as unknown as RunnerManager });
    try {
      const response = await app.inject({ method: "POST", url: "/api/runs", payload: {
        scenarioId: "fixture-scenario", mode, browserMode: "headful", prompt: "Complete fixture.",
      } });
      expect(response.statusCode).toBe(400);
  ...
  it("allows both local console ports and denies foreign browser origins before execution", async () => {
    const startRun = vi.fn();
    const app = createServer({ manager: { startRun, shutdown: async () => undefined } as unknown as RunnerManager });
    try {
      for (const origin of ["http://127.0.0.1:3000", "http://localhost:3041", "http://[::1]:3000"]) {
  ...
  it("replays a persisted terminal run with allowed-origin SSE headers", async () => {
    ...
    const detail = runDetailSchema.parse({
      run: { id: "test-run", scenarioId: scenario.id, labId: scenario.labId, mode: "native", browserMode: "headless", model: "test", prompt: "test", status: "completed", startedAt: "2026-09-03T00:00:00.000Z" },
    ...
    try {
      const response = await app.inject({ method: "GET", url: "/api/runs/test-run/events", headers: { origin: "http://127.0.0.1:3041" } });
      expect(response.statusCode).toBe(200);
      expect(response.headers["access-control-allow-origin"]).toBe("http://127.0.0.1:3041");
...
it("ends live SSE clients when the server closes", async () => {
  const scenario = listScenarios()[0]!;
  const detail = runDetailSchema.parse({
    run: { id: "test-run", scenarioId: scenario.id, labId: scenario.labId, browserMode: "headful", model: "test", prompt: "test", status: "running", startedAt: "2026-09-03T00:00:00.000Z" },
    scenario, workspacePath: "/tmp/test", eventStreamUrl: "/api/runs/test-run/events", replayUrl: "/api/runs/test-run/replay",
  ...
  it("returns a JSON error if a stored running replay has no live subscription", async () => {
    const scenario = listScenarios()[0]!;
    const detail = runDetailSchema.parse({
      run: { id: "orphaned-run", scenarioId: scenario.id, labId: scenario.labId, browserMode: "headless", model: "test", prompt: "test", status: "running", startedAt: "2026-09-03T00:00:00.000Z" },
  ...
  it.each(["before subscription", "during the fresh snapshot"])("includes completion %s exactly once and closes SSE", async (timing) => {
    ...
    const progress: RunEvent = { ...started, id: "race:1", sequence: 1, type: "run_progress" };
    const completed: RunEvent = { ...started, id: "race:2", sequence: 2, type: "run_completed", message: "Done" };
    const initial = runDetailSchema.parse({
      run: { id: "race", scenarioId: scenario.id, labId: scenario.labId, browserMode: "headless", model: "test", prompt: "test", status: "running", startedAt: started.createdAt },
    ...
    const getRunDetail = vi.fn()
      ...
      .mockImplementationOnce(async () => {
        if (timing === "during the fresh snapshot") {
          listener?.(completed);
          listener?.(progress);
          // A stale snapshot overlaps the buffered progress event.
          return { ...initial, events: [started, progress] };
        }
        return { ...initial, run: { ...initial.run, status: "completed" }, events: [started, progress, completed] };
      });
    const app = createServer({ manager: {
      getRunDetail,
    ...
    try {
      ...
      expect(events.map((event: RunEvent) => event.type)).toEqual(["run_started", "run_progress", "run_completed"]);
      expect(unsubscribe).toHaveBeenCalledOnce();
    } finally { await app.close(); }
  });

it("returns the active run for refresh, then null after Stop", async () => {
  const root = await mkdtemp(join(tmpdir(), "cua-active-run-"));
  ...
  try {
    ...
    const response = await app.inject({ method: "GET", url: "/api/runs/active" });
    expect(response.statusCode).toBe(200);
    expect(runDetailSchema.parse(response.json())).toEqual(started);
    await app.inject({ method: "POST", url: `/api/runs/${started.run.id}/stop` });
    expect((await app.inject({ method: "GET", url: "/api/runs/active" })).json()).toBeNull();
  } finally {
    await app.close();
