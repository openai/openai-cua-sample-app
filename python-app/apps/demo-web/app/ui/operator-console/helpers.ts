...
export const runnerUnavailableHint =
  "Start `pnpm dev` from `python-app`, check the runner terminal, then refresh the page.";
...
function titleForIssueCode(code: string) {
  switch (code) {
    ...
    case "invalid_verification_prompt":
      return "Verification needs a structured prompt";
    default:
...
function parseToolPayload(detail: string | undefined) {
  ...
  try {
    ...
    const code =
      typeof payload.code === "string" && label === "exec_py"
        ? payload.code
        : undefined;
