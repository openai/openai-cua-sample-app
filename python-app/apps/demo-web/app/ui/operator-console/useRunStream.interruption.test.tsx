...
describe("bounded console recovery", () => {
  ...
  it("shows a definitive desktop cleanup rejection without checking or retrying Start", async () => {
    const error = "Desktop input cleanup failed. New runs are blocked.";
    const hint = "Release held keys and mouse buttons, check desktop input permissions, then restart the runner before starting another run.";
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response({
      code: "desktop_cleanup_failed", error, hint,
    }, 503)));
    renderConsole();
