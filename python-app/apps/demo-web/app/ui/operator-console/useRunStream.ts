...
export function useRunStream({
  ...
  const handleStartRun = async () => {
    ...
    try {
      ...
      if (!isCurrentAction(request)) return;
      // This admission gate rejects before creating a run. Keep its recovery
      // instructions visible instead of treating every 503 as a lost response.
      const desktopCleanupRejected = !startAccepted && error instanceof RunnerApiError &&
        error.issue.code === "desktop_cleanup_failed";
      const ambiguous = !desktopCleanupRejected && (startAccepted || !(error instanceof RunnerApiError) ||
