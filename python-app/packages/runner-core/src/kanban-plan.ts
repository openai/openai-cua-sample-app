...
function parseBoardLine(line: string, targetState: KanbanBoardState) {
  ...
  const cardTokens = !rawCards || /^(?:none|empty|\[\])$/i.test(rawCards)
    ? []
    : rawCards.split(/\s*(?:->|,)\s*/g);
  const cardIdsForColumn = cardTokens.map((token) => {
    const cardId = resolveCardId(token);

    if (!cardId) {
      throw new Error(`Kanban prompt references unknown card "${token}".`);
    }
