...
import { type BrowserSession, type ModelObservation, PythonRuntimeError, pythonDesktopCapabilities } from "@cua-sample/browser-runtime";
import {
...
const defaultRunModel = process.env.CUA_DEFAULT_MODEL ?? "gpt-5.6";
...
export class RunnerManager {
  ...
  private desktopRecoveryRequired = false;
  ...
  async startRun(input: StartRunRequest): Promise<RunDetail> {
    ...
    if (this.desktopRecoveryRequired) {
      throw new RunnerCoreError("Desktop input cleanup failed. New runs are blocked.", {
        code: "desktop_cleanup_failed",
        hint: "Release held keys and mouse buttons, check desktop input permissions, then restart the runner before starting another run.",
        statusCode: 503,
      });
    }
    ...
    if (request.browserMode === "headless" && !pythonDesktopCapabilities.headless) {
      throw new RunnerCoreError("Local PyAutoGUI requires a visible browser.", {
        code: "visible_browser_required",
        hint: "Use browserMode: headful for local PyAutoGUI execution.",
        statusCode: 400,
      });
    }
    const browserMode = request.browserMode ?? "headful";
    ...
    // Reserve the desktop before the first asynchronous workspace operation.
    ...
    try {
      ...
      await this.emitEvent(context, {
        detail: `${scenario.title} · ${browserMode} · ${runRecord.maxResponseTurns ?? defaultMaxResponseTurns} turns`,
        level: "ok",
  ...
  async getReplayBundle(runId: string): Promise<ReplayBundle> {
    ...
    try {
      const replay = JSON.parse(await readFile(replayJsonPath, "utf8")) as ReplayBundle;
      return {
        ...replay,
        version: 2,
        // Strip obsolete execution fields while preserving historical Native events.
        run: runRecordSchema.parse(replay.run),
        scenario: scenarioManifestSchema.parse(replay.scenario),
        events: replay.events.map((event) => runEventSchema.parse(event)),
        browser: replay.browser ? browserStateSchema.parse(replay.browser) : undefined,
      };
    } catch (error) {
  ...
  private async captureScreenshot(
    ...
    const snapshot = observation ? await (async () => {
      const id = `observation-${randomUUID()}`;
      const path = join(this.getRunScreenshotsDirectory(context.detail.run.id), `${id}.png`);
      await writeFile(path, Buffer.from(observation.base64, "base64"));
      return { ...(await session.readState()), id, label, path, mimeType: "image/png" as const, capturedAt: this.now().toISOString() };
    })() : await session.captureScreenshot(label);
    ...
    const artifact = browserScreenshotArtifactSchema.parse({
      ...
      source: observation?.source ?? "browser_preview",
      imageWidth: observation?.width ?? (observation ? undefined : session.viewport.width),
      imageHeight: observation?.height ?? (observation ? undefined : session.viewport.height),
  ...
  private async executeRun(context: InternalRunContext) {
    try {
      ...
      await executor.execute({
        captureScreenshot: (session, label, observation) =>
          this.captureScreenshot(context, session, label, observation),
      ...
      const desktopCleanupFailed = error instanceof RunnerCoreError && error.code === "desktop_cleanup_failed";
      const cleanupFailed = desktopCleanupFailed || (error instanceof RunnerCoreError && error.code === "cleanup_failed");
      if (desktopCleanupFailed) this.desktopRecoveryRequired = true;
      if (context.abortController.signal.aborted && !cleanupFailed) return;
      context.finalizing = true;
      if (error instanceof PythonRuntimeError && error.code === "python_failsafe") {
        await this.publishTerminalRun(context, {
          notes: ["Desktop fail-safe activated."], outcome: "partial", status: "cancelled", verificationPassed: false,
        }, {
          detail: "python_failsafe", level: "warn", message: "Desktop fail-safe activated. Run cancelled.", type: "run_cancelled",
        });
        return;
      }
  ...
  private async failRun(context: InternalRunContext, error: unknown) {
    ...
    const notes = [
      ...
      ...(error instanceof PythonRuntimeError ? [`Error code: ${error.code}`] : []),
