...
const verificationPromptHint =
  "Use backlog:, in_progress:, and done: lines with each card exactly once; leave empty columns blank. See the Kanban example in labs/docs/scenarios.md, or turn verification off for a free-form prompt.";

class KanbanCodeExecutor implements RunExecutor {
  async execute(context: RunExecutionContext) {
    validateVerificationPrompt(context, parseKanbanTargetBoardState, verificationPromptHint);
