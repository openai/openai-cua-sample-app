...
const verificationPromptHint =
  "Supply hotel:, check_in:, check_out:, guest_name:, guest_email:, and special_request: lines. See the Booking example in labs/docs/scenarios.md, or turn verification off for a free-form prompt.";

class BookingCodeExecutor implements RunExecutor {
  async execute(context: RunExecutionContext) {
    validateVerificationPrompt(context, parseBookingRequest, verificationPromptHint);
