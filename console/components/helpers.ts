...
export function browserHelpText(backendId: "javascript" | "python") {
  return backendId === "python"
    ? "The app opens a visible browser and uses PyAutoGUI to control your desktop's mouse and keyboard. Keep the lab window in front during a run."
...
export const runnerUnavailableHint =
  "Start the selected backend from the repository root, check the runner terminal, then refresh the page.";
...
function parseToolPayload(detail: string | undefined) {
  ...
  try {
    ...
    const code =
      typeof payload.code === "string" && (label === "exec_js" || label === "exec_py")
        ? payload.code
        : undefined;
...
function summarizeToolCall(label: string, payload: Record<string, unknown>) {
  switch (label) {
    ...
    case "exec_py":
      return "Model requested code in the local Python runtime.";
    default:
      return Object.keys(payload).length > 0
