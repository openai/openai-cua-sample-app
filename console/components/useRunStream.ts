...
export function useRunStream({
  ...
  const defaultMaxResponseTurns = capabilities?.defaults.maxResponseTurns ?? 24;
  const initialScenario = scenarios.find((scenario) => scenario.id === initialRun?.run.scenarioId) ?? scenarios[0] ?? null;
  const [selectedScenarioId, setSelectedScenarioId] = useState(
    initialScenario?.id ?? "",
  );
  const [browserMode, setBrowserMode] = useState<BrowserMode>(initialRun?.run.browserMode ?? capabilities?.defaults.browserMode ?? "headless");
  ...
  const [workspaceState, setWorkspaceState] =
    useState<ScenarioResetResponse | null>(null);
  ...
  const selectedScenario =
    ...
    initialScenario;
  const backendMismatch = capabilities?.backendId !== expectedBackend || actionIssue?.code === "backend_mismatch";
  const runnerOnline = !initialRunnerIssue && !backendMismatch && scenarios.length > 0;
  const selectedRun =
    !backendMismatch && activeRun && selectedScenario && activeRun.run.scenarioId === selectedScenario.id
      ? activeRun
      : null;
  ...
  const capabilityIssue = capabilities && capabilities.backendId !== expectedBackend
    ? createRunnerIssue("backend_mismatch", `Expected ${expectedBackend}, but the runner is ${capabilities.backendId}.`, "Restart the console with the selected backend.")
    : null;
  const currentIssue = capabilityIssue ?? (actionIssue?.code === "backend_mismatch" ? actionIssue : runIssue ?? actionIssue ?? initialRunnerIssue);
  ...
  const fetchRunDetail = useCallback(
    async (runId: string, signal: AbortSignal) =>
      requestJson(
        ...
        runDetailSchema,
        { signal, headers: { "X-CUA-Backend": expectedBackend } },
        createFallbackIssue(
          `Run detail request failed for ${runId}.`,
          "Refresh the page or start a new run.",
        ),
      ),
    [runnerBaseUrl, expectedBackend],
  );
  ...
  useEffect(() => {
    ...
    const refreshDetail = () => {
      ...
      void fetchRunDetail(selectedRunId, controller.signal)
        ...
        .catch((error: unknown) => {
          if (!disposed && error instanceof RunnerApiError && error.issue.code === "backend_mismatch") {
            setActionIssue(error.issue);
          }
        })
        .finally(() => {
          refreshing = false;
  ...
  function adoptRun(detail: RunDetail) {
    ...
    setMaxResponseTurns(detail.run.maxResponseTurns);
  ...
  async function reconcileStart(request: ActionRequest) {
    ...
    try {
      const detail = await requestJson(
        ...
        { signal: request.controller.signal, headers: { "X-CUA-Backend": expectedBackend } },
      ...
      if (detail) {
        ...
        appendManualTranscript(createManualTranscript(
          "runner", `Recovered run ${detail.run.id}.`,
        ));
      } else {
      ...
      if (error instanceof RunnerApiError && error.issue.code === "backend_mismatch") setActionIssue(error.issue);
  ...
  const handleStartRun = async () => {
    ...
    try {
      const started = await requestJson(
        ...
        startRunResponseSchema,
        {
          body: JSON.stringify({
            browserMode,
            maxResponseTurns,
            model: capabilities?.defaults.model,
            prompt,
            scenarioId: selectedScenario.id,
          }),
          headers: {
            "Content-Type": "application/json",
            "X-CUA-Backend": expectedBackend,
          },
          method: "POST",
      ...
      if (!isCurrentAction(request)) return;
      const definitiveRejection = error instanceof RunnerApiError &&
        (error.issue.code === "backend_mismatch" || error.issue.code === "desktop_cleanup_failed");
      const ambiguous = !definitiveRejection && (!(error instanceof RunnerApiError) ||
  ...
  const handleStopRun = async () => {
    ...
    try {
      const detail = await requestJson(
        ...
        {
          method: "POST",
          headers: { "X-CUA-Backend": expectedBackend },
          signal: request.controller.signal,
        },
        createFallbackIssue(
          "Run stop failed.",
    ...
    try {
      ...
      appendManualLog(
        createManualLog(
          ...
          "Scenario reset. The next run starts from a fresh template.",
          "ok",
        ),
      );
      appendManualTranscript(
        createManualTranscript(
          "runner",
          "The selected scenario is ready for a fresh run.",
        ),
      );
