...
import { scenarioFixture as scenario } from "./fixtures";
...
const makeDetail = (id = "demo-run"): RunDetail => ({
  run: { maxResponseTurns: 24, id, scenarioId: scenario.id, labId: scenario.labId, browserMode: "headful", model: "test-model", prompt: "Complete the demo task.", status: "running", startedAt: "2026-09-03T00:00:00.000Z" },
...
function mount(initialRun: RunDetail | null = null) {
  return renderHook(() => useRunStream({ ...javascriptBackendProps, initialRun, initialRunnerIssue: null, runnerBaseUrl: baseUrl, scenarios: [scenario] }));
}
function renderConsole() {
  return render(<OperatorConsole {...javascriptBackendProps} initialRunnerIssue={null} runnerBaseUrl={baseUrl} scenarios={[scenario]} />);
}
...
describe("bounded console recovery", () => {
  ...
  it("reconciles an invalid Start response without repeating POST", async () => {
    const detail = makeDetail();
    vi.stubGlobal("fetch", vi.fn()
      .mockResolvedValueOnce(response({ unexpected: "payload" }, 202))
      .mockResolvedValueOnce(response(detail)));
    const hook = mount();
    await act(async () => { await hook.result.current.handleStartRun(); });
    expect(hook.result.current.selectedRun?.run.id).toBe(detail.run.id);
    expect(vi.mocked(fetch).mock.calls.map(([url]) => url)).toEqual([
      `${baseUrl}/api/runs`, `${baseUrl}/api/runs/active`,
    ]);
  });
  ...
  it("aborts obsolete actions and ignores their late successful replies", async () => {
    ...
    const hook = renderHook(({ url }) => useRunStream({ ...javascriptBackendProps, initialRunnerIssue: null, runnerBaseUrl: url, scenarios: [scenario] }), { initialProps: { url: baseUrl } });
